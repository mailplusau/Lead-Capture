 ...
 * @Last modified time: 2021-11-04T10:16:34+11:00
 */
  ...
  function (ui, email, runtime, search, record, http, log, redirect, format) {
    ...
    function onRequest(context) {
      if (context.request.method === 'GET') {
        ...
        inlineHtml +=
          '<input id="customer_id" class="form-control" required value="' +
          customer_id + '" type="hidden"/></div></div>';
        ...
        inlineHtml +=
          '<input id="zee_id" class="form-control" required value="' + zee +
          '" type="hidden"/></div></div>';
        ...
        // Customer Details
        inlineHtml += customerDetailsSection(entityid, companyName, abn,
          resultSetZees, zee, accounts_email, daytodayphone, daytodayemail,
          accounts_phone, customer_status, lead_source, old_zee,
          old_zee_text, old_customer, customer_industry, lead_source_text,
          customer_status_id, account_manager);
        ...
        if (!isNullorEmpty(serviceAddressResult)) {
          if (serviceAddressResult.length > 0) {
            ...
            if (isFinanceRole(role) && customer_status_id == 13) {

              // inlineHtml += selectorSection(entityid);
              inlineHtml += maapBankAccountSection(maap_bank_account_number,
                maap_parent_bank_account_number, selector_type);

              inlineHtml += franchiseeMainContactSection(franchisee_name,
                zee_main_contact_name, zee_email, zee_main_contact_phone,
                zee_abn);
              // inlineHtml += enquiryHeader();
              inlineHtml += otherInvoiceFieldsSection(
                selected_invoice_method_id, accounts_cc_email,
                mpex_po_number, customer_po_number,
                selected_invoice_cycle_id, terms, customer_terms,
                status_value, selector_type);
              inlineHtml += openInvoicesSection(ticket_id, selector_type);
            }

            inlineHtml +=
              '<div class="tabs" style="font-size: xx-small;"><ul class="nav nav-tabs nav-justified" style="padding-top: 3%;border-bottom: 1px solid white;">';
            var tab_content = '';
            inlineHtml +=
            ...
            inlineHtml +=
              '<li role="presentation" class=""><a href="#survey"><b>ADDITIONAL INFORMATION</b></a></li>';
            ...
            tab_content += surveyInfo(ap_mail_parcel, ap_outlet, lpo_customer,
              multisite, website, zee_visit_notes, zee_visit,
              ap_mail_parcel, ap_outlet, lpo_customer, using_express_post,
              using_local_couriers, using_po_box, bank_visit, classify_lead
            );
            tab_content += '</div>';
        ...
        if (customer_status_id == 13 || customer_status_id == 32) {
          ...
          if (!isNullorEmpty(serviceContactResult) && !isNullorEmpty(
            serviceAddressResult)) {
            if (serviceContactResult.length > 0 && serviceAddressResult.length >
              0) {
              form.addButton({
                id: 'back',
                label: 'SEND EMAIL',
        ...
        if (customer_status_id != 13) {
          ...
          if (runtime.getCurrentUser() != 696992 && auto_allocate == 1 &&
            customer_list != 'T') {
            ...
            if (status_id == '57') {
              ...
              email.send({
                ...
                body: body,
                recipients: salesRep,
                subject: 'Sales HOT Lead - ' + entity_id + ' ' +
                  customer_name,
        ...
        if (customer_status_id == 13) { //If customer Status is Customer - Signed redirect to ther Customer List Page
          ...
          if (script_id == 'customscript_sl_mpex_new_cust_last_4m') {
            redirect.toSuitelet({
              scriptId: 'customscript_sl_mpex_new_cust_last_4m',
              deploymentId: 'customdeploy1',
            });
          } else if (script_id == 'customscript_sl_customer_list') {
            redirect.toSuitelet({
    ...
    */
    function customerDetailsSection(entityid, companyName, abn, resultSetZees,
      zee, accounts_email, daytodayphone, daytodayemail, accounts_phone,
      customer_status, lead_source, old_zee, old_zee_text, old_customer,
      customer_industry, lead_source_text, customer_status_id, account_manager) {
      var inlineQty = '';
      ...
      if (!isNullorEmpty(entityid)) {
        ...
        inlineQty += '<div class="row">';
        inlineQty +=
          '<div class="col-xs-6 entityid"><div class="input-group"><span class="input-group-addon" id="entityid_text">ID </span><input id="entityid" class="form-control entityid" readonly value="' +
          entityid + '" data-oldvalue="' + entityid + '" /></div></div>';
        inlineQty += '</div>';
        inlineQty += '</div>';
      }
      ...
      inlineQty += '<div class="row">';
      inlineQty +=
        '<div class="col-xs-6 company_name"><div class="input-group"><span class="input-group-addon" id="company_name_text">COMPANY NAME <span class="mandatory">*</span></span><input id="company_name" class="form-control company_name" required value="' +
        companyName + '" data-oldvalue="' + companyName + '" /></div></div>';
      inlineQty +=
        '<div class="col-xs-6 industry"><div class="input-group"><span class="input-group-addon" id="industry_text">INDUSTRY <span class="mandatory">*</span></span><select id="industry" class="form-control industry"><option value="19">OTHER SERVICES</option>';
      ...
      industryRes.each(function (industrySearch) {
        ...
        if (!isNullorEmpty(customer_industry)) {
          if (customer_industry == listID) {
            inlineQty += '<option value="' + listID + '" selected>' +
              listValue + '</option>';
          }
        }
      ...
      inlineQty += '<div class="row">';
      inlineQty +=
        '<div class="col-xs-6 abn"><div class="input-group"><span class="input-group-addon" id="abn_text">ABN </span><input id="abn" class="form-control abn" value="' +
      ...
      if (role != 1000) {
        inlineQty +=
          '<div class="col-xs-6 status"><div class="input-group"><span class="input-group-addon" id="status_text">STATUS <span class="mandatory">*</span></span>';
        if (customer_status_id == 13 || customer_status_id == 32) {
          inlineQty +=
            '<select id="status" class="form-control status" readonly><option></option>';
        } else {
          inlineQty +=
            '<select id="status" class="form-control status"><option></option>';
        }


        if (customer_status_id == 6) {
          inlineQty += '<option value="' + 6 +
            '" selected>SUSPECT - New</option>';
        } else if (customer_status_id == 57) {
          inlineQty += '<option value="' + 57 +
            '" selected>SUSPECT - Hot Lead</option>';
        } else if (customer_status_id == 13 || customer_status_id == 32) {
          inlineQty += '<option value="' + customer_status_id +
            '" selected >' + customer_status + '</option>';
        }
        inlineQty += '<option value="' + 6 + '" >SUSPECT - New</option>';
        inlineQty += '<option value="' + 57 +
          '" >SUSPECT - Hot Lead</option>';
        ...
        inlineQty +=
          '<div class="col-xs-6 status"><div class="input-group"><span class="input-group-addon" id="status_text">STATUS <span class="mandatory">*</span></span><select id="status" class="form-control status" readonly><option value="' +
          6 + '" selected>SUSPECT - New</option>';
      ...
      inlineQty += '<div class="row">';
      inlineQty +=
        '<div class="col-xs-6 zee"><div class="input-group"><span class="input-group-addon" id="zee_text">FRANCHISEE <span class="mandatory">*</span></span>';
      ...
      if (role == 1000) {
        ...
        inlineQty +=
          '<select id="zee" readonly class="form-control zee" ><option value="' +
          zee + '" selected>' + zee_name + '</option>';
      } else {
        //For all other roles, Franchisee is chosen
        inlineQty +=
          '<select id="zee" class="form-control zee" ><option value=0></option>';
        resultSetZees.each(function (searchResultZees) {
          ...
          zee_main_contact_phone = searchResultZees.getValue(
            'custentity2');
          zee_abn = searchResultZees.getValue(
            'custentity_abn_franchiserecord');

          if (zeeId == zee) {
            inlineQty += '<option value="' + zeeId + '" selected>' +
              zeeName + '</option>';
          } else {
            inlineQty += '<option value="' + zeeId + '">' + zeeName +
              '</option>';
          }
      ...
      if (role == 1000) {
        ...
        if (zee != 696179) {
          inlineQty +=
            '<select id="leadsource" class="form-control leadsource" readonly ><option value="-4"  selected>Franchisee Generated</option>';
        } else {
          inlineQty +=
            '<select id="leadsource" class="form-control leadsource" readonly ><option value="242647"  selected>Field Sales - Wendie</option>';
        }
      ...
      if (role == 1032) {
        ...
        inlineQty +=
          '<div class="form-group container relocation_section hide">';
        inlineQty += '<div class="row">';
        inlineQty +=
          '<div class="col-xs-6 old_zee"><div class="input-group"><span class="input-group-addon" id="zee_text">OLD FRANCHISEE</span><input id="old_zee" class="form-control old_cust" value="' +
          old_zee_text + '" data-oldvalue="' + old_zee_text + '" data-id="' +
          old_zee + '" readonly/>';
        inlineQty += '</div></div>';
        inlineQty +=
          '<div class="col-xs-6"><div class="input-group"><span class="input-group-addon" id="cust_text">OLD CUSTOMER</span><input id="old_cust" type="number" class="form-control old_cust" value="' +
          old_customer + '" data-oldvalue="' + old_customer +
          '"/></div></div>';
        inlineQty += '</div>';
        inlineQty += '</div>';
      }
      ...
      inlineQty += '<div class="row">';
      inlineQty +=
        '<div class="col-xs-6 account_email_div"><div class="input-group"><span class="input-group-addon" id="account_email_text">ACCOUNTS (MAIN) EMAIL</span><input id="account_email" type="email" class="form-control account_email" data-oldvalue="' +
        accounts_email + '" value="' + accounts_email + '" /></div></div>';
      inlineQty +=
        '<div class="col-xs-6 daytodayemail_div"><div class="input-group"><span class="input-group-addon" id="daytodayemail_text">DAY-TO-DAY EMAIL </span><input id="daytodayemail" type="email" class="form-control daytodayemail" data-oldvalue="' +
      ...
      inlineQty +=
        '<div class="col-xs-6 daytodayphone_div"><div class="input-group"><span class="input-group-addon" id="daytodayphone_text">DAY-TO-DAY PHONE </span><input id="daytodayphone" class="form-control daytodayphone" data-oldvalue="' +
      ...
      inlineQty += '<div class="row">';
      inlineQty +=
        '<div class="col-xs-6 account_manager_div"><div class="input-group"><span class="input-group-addon" id="account_manager_span">ACCOUNT MANAGER</span><select id="account_manager" class="form-control account_manager" ><option></option>';
    ...
    function selectorSection(entityid) {
      ...
      inlineQty +=
        '<h4><span class="label label-default col-xs-12" style="background-color: #095c7b;">TICKET DETAILS</span></h4>';
      inlineQty += '</div></div></div>';
      return inlineQty;
    }

    function maapBankAccountSection(maap_bank_account_number,
      maap_parent_bank_account_number, selector_type) {
      var inlineQty =
        '<div class="form-group container accounts_number_section">';
      inlineQty += '<div class="row">';
      ...
      inlineQty +=
        '<span class="input-group-addon" id="account_number_text">MAAP BANK ACCOUNT #</span>';
      inlineQty += '<input id="account_number" type="number" value="' +
        maap_bank_account_number +
        '" class="form-control account_number" disabled />';
      inlineQty += '</div></div>';
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="parent_account_number_text">MAAP PARENT BANK ACCOUNT #</span>';
      inlineQty += '<input id="parent_account_number" type="number" value="' +
        maap_parent_bank_account_number +
        '" class="form-control parent_account_number" disabled />';
     ...
     */
    function franchiseeMainContactSection(franchisee_name,
      zee_main_contact_name, zee_email, zee_main_contact_phone, zee_abn) {
      ...
      var inlineQty =
        '<div class="form-group container zee_main_contact_section">';
      inlineQty += '<div class="row">';
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="franchisee_name_text">FRANCHISEE NAME</span>';
      inlineQty += '<input id="franchisee_name" value="' + franchisee_name +
        '" class="form-control franchisee_name" disabled>';
      inlineQty += '</div></div>';
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="zee_main_contact_name_text">MAIN CONTACT</span>';
      inlineQty += '<input id="zee_main_contact_name" value="' +
        zee_main_contact_name +
        '" class="form-control zee_main_contact_name" disabled>';
      inlineQty += '</div></div></div></div>';
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="zee_email_text">FRANCHISEE EMAIL</span>';
      inlineQty += '<input id="zee_email" type="email" value="' + zee_email +
        '" class="form-control accountsemail" disabled />';
      inlineQty += '<div class="input-group-btn">';
      var zee_contact_id = '0';
      inlineQty +=
        '<button type="button" class="btn btn-success add_as_recipient" data-email="' +
        zee_email + '" data-contact-id="' + zee_contact_id +
        '" data-firstname="' + franchisee_name +
        '" data-toggle="tooltip" data-placement="right" title="Add as recipient">';
      ...
      inlineQty += '<div class="input-group">'
      inlineQty +=
        '<span class="input-group-addon" id="zee_main_contact_phone_text">FRANCHISEE PHONE</span>';
      inlineQty += '<input id="zee_main_contact_phone" type="tel" value="' +
        zee_main_contact_phone +
        '" class="form-control zee_main_contact_phone" disabled />';
      inlineQty +=
        '<div class="input-group-btn"><button type="button" class="btn btn-success" id="call_zee_main_contact_phone"><span class="glyphicon glyphicon-earphone"></span></button>';
      inlineQty += '</div>';
      inlineQty += '</div></div>';
      ...
      inlineQty += '<div class="input-group">'
      inlineQty +=
        '<span class="input-group-addon" id="zee_abn_text">FRANCHISEE ABN</span>'
      inlineQty += '<input id="zee_abn" class="form-control zee_abn" value="' +
        zee_abn + '" disabled>'
      inlineQty += '</div></div></div></div>';
    ...
    function enquiryHeader() {
      ...
      var inlineQty =
        '<div class="form-group container ticket_enquiry_header_section">';
      inlineQty += '<div class="row">';
      inlineQty += '<div class="col-xs-12 heading2">';
      inlineQty +=
        '<h4><span class="label label-default col-xs-12" style="background-color: #095c7b;">TICKET ENQUIRY DETAILS</span></h4>';
      inlineQty += '</div></div></div>';
      return inlineQty
    }
    /**
     ...
     * @return  {String} inlineQty
     */
    function otherInvoiceFieldsSection(selected_invoice_method_id,
      accounts_cc_email, mpex_po_number, customer_po_number,
      selected_invoice_cycle_id, terms, customer_terms, status_value,
      selector_type) {
      ...
      inlineQty =
        '<div class="form-group container invoice_method_accounts_cc_email_section">';
      inlineQty += '<div class="row">';
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="invoice_method_text">INVOICE METHOD</span>';
      inlineQty += '<select id="invoice_method" class="form-control" ' +
        disabled + '>';
      ...
      invoiceMethodResultSet.forEach(function (invoiceMethodResult) {
        ...
        if (invoice_method_id == selected_invoice_method_id) {
          inlineQty += '<option value="' + invoice_method_id +
            '" selected>' + invoice_method_name + '</option>';
        } else {
          inlineQty += '<option value="' + invoice_method_id + '">' +
            invoice_method_name + '</option>';
        }
      });
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="accounts_cc_email_text">ACCOUNTS CC EMAIL</span>';
      inlineQty += '<input id="accounts_cc_email" type="email" value="' +
        accounts_cc_email + '" class="form-control accounts_cc_email"  ' +
        disabled + '/>';
      inlineQty += '<div class="input-group-btn">';
      inlineQty +=
        '<button type="button" class="btn btn-success add_as_recipient" data-email="' +
      ...
      inlineQty +=
        '<div class="form-group container mpex_customer_po_number_section">';
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="mpex_po_number_text">MPEX PO #</span>';
      inlineQty += '<input id="mpex_po_number" value="' + mpex_po_number +
        '" class="form-control mpex_po_number"  ' + disabled + '/>';
      inlineQty += '</div></div>';
      // Customer PO #
      inlineQty += '<div class="col-xs-6 customer_po_number_div">';
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="customer_po_number_text">CUSTOMER PO #</span>';
      inlineQty += '<input id="customer_po_number" value="' +
        customer_po_number + '" class="form-control customer_po_number"  ' +
        disabled + '/>';
      inlineQty += '</div></div></div></div>';
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="terms_text">TERMS</span>';
      ...
      inlineQty += '<input id="terms" class="form-control terms" value="' +
        terms_text + '" disabled/>';
      inlineQty += '</div></div>';
      ...
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="customers_terms_text">' +
        "CUSTOMER'S TERMS</span>";
      inlineQty +=
        '<input id="customers_terms" class="form-control customers_terms" value="' +
        customer_terms + '" ' + disabled + '/>';
      inlineQty += '</div></div></div></div>';
      ...
      inlineQty +=
        '<div class="form-group container mpex_invoicing_cycle_section">';

      inlineQty += '<div class="row">';
      inlineQty += '<div class="col-xs-12 mpex_invoicing_cycle_div">';
      inlineQty += '<div class="input-group">';
      inlineQty +=
        '<span class="input-group-addon" id="mpex_invoicing_cycle_text">MPEX INVOICING CYCLE</span>';
      inlineQty +=
        '<select id="mpex_invoicing_cycle" class="form-control mpex_invoicing_cycle" ' +
        disabled + '>';
      inlineQty += '<option></option>';
      ...
      invoiceCycleResultSet.each(function (invoiceCycleResult) {
        ...
        if (invoice_cycle_id == selected_invoice_cycle_id) {
          inlineQty += '<option value="' + invoice_cycle_id +
            '" selected>' + invoice_cycle_name + '</option>';
        } else {
          inlineQty += '<option value="' + invoice_cycle_id + '">' +
            invoice_cycle_name + '</option>';
        }
      });
      inlineQty += '</select>';
      inlineQty += '</div></div></div></div>';
    ...
    */
    function addressContactsSection(resultSetAddresses, resultSetContacts) {
      ...
      inlineQty += '<div class="row">';
      inlineQty +=
        '<div class="col-xs-6 heading3"><h4><span class="label label-default col-xs-12" style="background-color: #095c7b;">ADDRESS DETAILS</span></h4></div>';
      inlineQty +=
        '<div class="col-xs-6 heading2"><h4><span class="label label-default col-xs-12" style="background-color: #095c7b;">CONTACT DETAILS</span></h4></div>';
      inlineQty += '</div>';
      inlineQty += '</div>';
      ...
      inlineQty +=
        '<table border="0" cellpadding="15" id="address" class="table table-responsive table-striped address tablesorter" cellspacing="0" style="width: 100%;font-size: 10px;"><thead style="color: white;background-color: #095c7b;"><tr><th style="vertical-align: middle;text-align: center;"><b>DETAILS</b></th><th style="vertical-align: middle;text-align: center;"><b>DEFAULT SHIPPING</b></th><th style="vertical-align: middle;text-align: center;"><b>DEFAULT BILLING</b></th><th style="vertical-align: middle;text-align: center;"><b>GEOCODED</b></th></tr></thead><tbody>';
      ...
      if (!isNullorEmpty(resultSetAddresses)) {
        ...
        resultSetAddresses.each(function (searchResultAddresses) {
          ...
          if (billing_error == 'F') {
            inlineQty += '<tr><td>' + full_address +
              '</td><td>' + default_shipping + '</td><td> ' + default_billing + '</td><td> NO </td></tr>';
          } else {
            inlineQty += '<tr><td>' + full_address +
              '</td><td>' + default_shipping + '</td><td> ' + default_billing + '</td><td> YES </td></tr>';
          }
      ...
      inlineQty +=
        '<table border="0" cellpadding="15" id="contacts" class="table table-responsive table-striped contacts tablesorter" cellspacing="0" style="width: 100%;font-size: 10px;"><thead style="color: white;background-color: #095c7b;"><tr><th style="vertical-align: middle;text-align: center;"><b>DETAILS</b></th><th style="vertical-align: middle;text-align: center;"><b>ROLE</b></th><th style="vertical-align: middle;text-align: center;"><b>PORTAL ADMIN</b></th><th style="vertical-align: middle;text-align: center;"><b>PORTAL USER</b></th></tr></thead><tbody>';
      if (!isNullorEmpty(resultSetContacts)) {
        resultSetContacts.each(function (searchResultContacts) {
          ...
          var contact_role_text = searchResultContacts.getText(
            'contactrole');
          var portal_admin = searchResultContacts.getText(
            'custentity_connect_admin');
          var portal_user = searchResultContacts.getText(
            'custentity_connect_user');

          inlineQty += '<tr class="text-center"><td>' + contact_text +
            '</td><td>' + contact_role_text + '</td><td>' + portal_admin + '</td><td>' + portal_user + '</td></tr>';
          return true;
        });
      }
      inlineQty += '</tbody></table>';
    ...
    */
    function apTotol(resultSetAPTotal) {
      ...
      resultSetAPTotal.each(function (searchResult) {
        var apTotolValue = searchResult.getValue(
          "custrecord_ap_order_exprevenue",
          "CUSTRECORD_AP_PRODUCT_ORDER", "SUM");
        var apTotolQty = searchResult.getValue(
          "custrecord_ap_stock_line_actual_qty", null, "SUM");

        inlineQty +=
          '<div class="col-xs-6 ap_total"><div class="input-group"><span class="input-group-addon" id="ap_total_text">TOTAL REVENUE ($)</span><input type="text" id="ap_total" class="form-control ap_total" value="' +
          apTotolValue + '" readonly /></div></div>';
        inlineQty +=
          '<div class="col-xs-6 ap_total_qty"><div class="input-group"><span class="input-group-addon" id="ap_total_qty_text">TOTAL QTY (10-Packs)</span><input type="text" id="ap_total_qty" class="form-control ap_total_qty" value="' +
          apTotolQty + '" readonly /></div></div>';
      ...
      if (count == 0) {
        ...
        inlineQty +=
          '<div class="col-xs-6 ap_total"><div class="input-group"><span class="input-group-addon" id="ap_total_text">TOTAL REVENUE ($)</span><input type="text" id="ap_total" class="form-control ap_total" value="0" readonly /></div></div>';
        inlineQty +=
          '<div class="col-xs-6 ap_total_qty"><div class="input-group"><span class="input-group-addon" id="ap_total_qty_text">TOTAL QTY (10-Packs)</span><input type="text" id="ap_total_qty" class="form-control ap_total_qty" value="0" readonly /></div></div>';
      }
      inlineQty += '</div>';
      inlineQty += '</div>';
    ...
    */
    function mpexTab(customer_id, mpex_customer,
      ...
      inlineQty += '<div class="form-group container exp_usage_section">';
      inlineQty += '<div class="row">';
      inlineQty +=
      ...
      inlineQty += '</div>';
      inlineQty += '</div>';
      inlineQty += '<div class="form-group container exp_usage2_section">';
      inlineQty += '<div class="row">';
      inlineQty +=
      ...
      inlineQty += '<div class="form-group container reviewprodpricing_section">';
      inlineQty += '<div class="row">';
      inlineQty += '<div class="col-xs-3 "></div>';
      inlineQty +=
        '<div class="col-xs-6 reviewprodpricing_div"><input type="button" value="ADD/EDIT PRODUCT PRICING" class="form-control btn btn-primary" id="reviewprodpricing" /></div>';
      inlineQty += '<div class="col-xs-3 "></div>';
      inlineQty += '</div>';
      inlineQty += '</div>';
      ...
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-12 heading1"><h4><span class="label label-default col-xs-12" style="background-color: #103D39;">MPEX - STOCK TO BE DROPPED OFF</span></h4></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      ...
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-2 drop_b4"><div class="input-group"><span class="input-group-addon" id="min_b4_text">B4 (10-Packs)</span><input id="drop_b4" class="form-control drop_b4"  value="2"  /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 drop_c5"><div class="input-group"><span class="input-group-addon" id="drop_c5_text">C5 (10-Packs)</span><input id="drop_c5" class="form-control drop_c5"  value="2"  /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 drop_dl"><div class="input-group"><span class="input-group-addon" id="drop_dl_text">DL (10-Packs)</span><input id="drop_dl" class="form-control drop_dl"  value="2"  /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 drop_1kg"><div class="input-group"><span class="input-group-addon" id="drop_1kg_text">1Kg (10-Packs)</span><input id="drop_1kg" class="form-control drop_1kg"  value="2"  /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 drop_3kg"><div class="input-group"><span class="input-group-addon" id="drop_3kg_text">3Kg (10-Packs)</span><input id="drop_3kg" class="form-control drop_3kg"  value="2" /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 drop_5kg"><div class="input-group"><span class="input-group-addon" id="drop_5kg_text">5Kg (10-Packs)</span><input id="drop_5kg" class="form-control drop_5kg"  value="2"  /></div></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      ...
      // inlineQty +=
      //   '<div class="col-xs-4 dropoffdate"><div class="input-group"><span class="input-group-addon" id="dropoffdate_text">DROP OFF DATE</span><input id="dropoffdate" class="form-control dropoffdate"  value="" type="date" /></div></div>';
      // inlineQty += '<div class="col-xs-4 "></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      ...
      // inlineQty += '<div class="col-xs-4 "></div>';
      // inlineQty +=
      //   '<div class="col-xs-4 contact_name"><div class="input-group"><span class="input-group-addon" id="contact_name_text">CONTACT NAME</span><input id="contact_name" class="form-control contact_name"  value="" type="text" /></div></div>';
      // inlineQty += '<div class="col-xs-4 "></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      ...
      //   inlineQty +=
      //     '<div class="col-xs-3 sendemail"><input type="button" value="FRANCHISEE NOTIFIED" class="form-control btn" id="sendemail" style=""/></div>';
      // } else {
      //   inlineQty +=
      //     '<div class="col-xs-3 sendemail"><input type="button" value="NOTIFY FRANCHISEE" class="form-control btn btn-primary" id="sendemail" style="background-color: #008657;"/></div>';
      // }
      ...
      //     0) {
      //     inlineQty +=
      //       '<div class="col-xs-3 "><input type="button" id="invitetoportal" class="form-control invitetoportal btn btn-success" value="INVITE TO PORTAL" onclick="onclick_InviteEmail();" style="background-color: #fdce0e;"/></div>';
      //     inlineQty +=
      //       '<div class="col-xs-3 "><input type="button" id="invitetoportal" class="form-control invitetoportal btn btn-success" value="INVITE TO PORTAL (U4)" onclick="onclick_InviteEmailU4();" style="background-color: #fdce0e;"/></div>';
      //     inlineQty +=
      //       '<div class="col-xs-3 "><input type="button" id="sendinfo" class="form-control sendInfo btn btn-primary" value="SEND INFO" onclick="onclick_SendInfo();" style=""/></div>';
      //   }
      // }
      ...
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-12 heading1"><h4><span class="label label-default col-xs-12" style="background-color: #103D39;">MPEX - PRICING STRUCTURE</span></h4></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      ...
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-12 heading2"><h5><span class="label label-default col-xs-12" style="background-color: #00808087;">MPEX - CURRENT PRICING STRUCTURE</span></h5></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      ...
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_b4"><div class="input-group"><span class="input-group-addon">B4</span><input class="form-control mpex_b4"  value="' +
      //   mpex_b4 + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_c5"><div class="input-group"><span class="input-group-addon">C5</span><input class="form-control mpex_c5"  value="' +
      //   mpex_c5 + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_dl"><div class="input-group"><span class="input-group-addon">DL</span><input class="form-control mpex_dl"  value="' +
      //   mpex_dl + '" readonly /></div></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      // inlineQty += '<div class="form-group container current_mpex_section">';
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_500g"><div class="input-group"><span class="input-group-addon">500g</span><input" class="form-control mpex_500g"  value="' +
      //   mpex_500g + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_1kg"><div class="input-group"><span class="input-group-addon">1Kg</span><input class="form-control mpex_1kg"  value="' +
      //   mpex_1kg + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_3kg"><div class="input-group"><span class="input-group-addon">3Kg</span><input class="form-control mpex_3kg"  value="' +
      //   mpex_3kg + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_5kg"><div class="input-group"><span class="input-group-addon">5Kg</span><input class="form-control mpex_5kg"  value="' +
      //   mpex_5kg + '" readonly /></div></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      ...
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-12 heading2"><h5><span class="label label-default col-xs-12" style="background-color: #00808087;">MPEX - SCHEDULED PRICING STRUCTURE</span></h5></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';

      // inlineQty +=
      //   '<div class="form-group container scheduled_mpex_section">';
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_b4_new"><div class="input-group"><span class="input-group-addon">B4</span><input class="form-control mpex_b4_new"  value="' +
      //   mpex_b4_new + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_c5_new"><div class="input-group"><span class="input-group-addon">C5</span><input class="form-control mpex_c5_new"  value="' +
      //   mpex_c5_new + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_dl_new"><div class="input-group"><span class="input-group-addon">DL</span><input class="form-control mpex_dl_new"  value="' +
      //   mpex_dl_new + '" readonly /></div></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      // inlineQty +=
      //   '<div class="form-group container scheduled_mpex_section">';
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_500g_new"><div class="input-group"><span class="input-group-addon">500g</span><input" class="form-control mpex_500g_new"  value="' +
      //   mpex_500g_new + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_1kg_new"><div class="input-group"><span class="input-group-addon">1Kg</span><input class="form-control mpex_1kg_new"  value="' +
      //   mpex_1kg_new + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_3kg_new"><div class="input-group"><span class="input-group-addon">3Kg</span><input class="form-control mpex_3kg_new"  value="' +
      //   mpex_3kg_new + '" readonly /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 mpex_5kg_new"><div class="input-group"><span class="input-group-addon">5Kg</span><input class="form-control mpex_5kg_new"  value="' +
      //   mpex_5kg_new + '" readonly /></div></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';

      // inlineQty +=
      //   '<div class="form-group container scheduled_date_section">';
      // inlineQty += '<div class="row">';
      // inlineQty += '<div class="col-xs-4 "></div>';
      // inlineQty +=
      //   '<div class="col-xs-4 scheduled_date"><div class="input-group"><span class="input-group-addon" id="dropoffdate_text">SCHEDULED DATE</span><input id="scheduled_date" class="form-control scheduled_date"  value="' +
      //   mpex_start_date + '" type="date" readonly /></div></div>';
      // inlineQty += '<div class="col-xs-4 "></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      ...
      resultSetCustomer.each(function (searchResult) {
        ...
        for (var x = 0; x < parsedUsage['Usage'].length; x++) {
          ...
          inlineQty += '<tr class="dynatable-editable">';
          inlineQty += '<td>' + parts[2] + '-' + ('0' + parts[1]).slice(-
      ...
      // inlineQty += '<div class="row">';
      // inlineQty +=
      //   '<div class="col-xs-12 heading1"><h4><span class="label label-default col-xs-12">MPEX - STOCK AT CUSTOMER</span></h4></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      //
      // inlineQty += '<div class="form-group container entityid_section">';
      ...
      //
      // inlineQty +=
      //   '<div class="col-xs-2 total_b4"><div class="input-group"><span class="input-group-addon" id="min_b4_text">B4 (Pieces) </span><input id="total_b4" class="form-control total_b4" readonly value="' +
      //   total_b4 + '" data-oldvalue="' + total_b4 + '" /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 total_c5"><div class="input-group"><span class="input-group-addon" id="total_c5_text">C5 (Pieces)</span><input id="total_c5" class="form-control total_c5" readonly value="' +
      //   total_c5 + '" data-oldvalue="' + total_b4 + '" /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 total_dl"><div class="input-group"><span class="input-group-addon" id="total_dl_text">DL (Pieces)</span><input id="total_dl" class="form-control total_dl"readonly  value="' +
      //   total_dl + '" data-oldvalue="' + total_dl + '" /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 total_1kg"><div class="input-group"><span class="input-group-addon" id="total_1kg_text">1Kg (Pieces) </span><input id="total_1kg" class="form-control total_1kg" readonly value="' +
      //   total_1kg + '" data-oldvalue="' + total_1kg + '" /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 total_3kg"><div class="input-group"><span class="input-group-addon" id="total_3kg_text">3Kg (Pieces) </span><input id="total_3kg" class="form-control total_3kg" readonly value="' +
      //   total_3kg + '" data-oldvalue="' + total_3kg + '" /></div></div>';
      // inlineQty +=
      //   '<div class="col-xs-2 total_5kg"><div class="input-group"><span class="input-group-addon" id="total_5kg_text">B4 (Pieces)</span><input id="total_5kg" class="form-control total_5kg" readonly value="' +
      //   total_5kg + '" data-oldvalue="' + total_5kg + '" /></div></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
    ...
    */
    function surveyInfo(ap_mail_parcel, ap_outlet, lpo_customer, multisite,
      website, zee_visit_notes, zee_visit, ap_mail_parcel, ap_outlet,
      lpo_customer, using_express_post, using_local_couriers, using_po_box,
      bank_visit, classify_lead) {
      ...
      inlineQty += '</select></div></div>';
      inlineQty +=
        '<div class="col-xs-6 website"><div class="input-group"><span class="input-group-addon" id="survey2_text">MULTISITE WEB LINK </span><input id="website" type="text" class="form-control website" /></div></div>';
      inlineQty += '</div>';
      inlineQty += '</div>';
      ...
      inlineQty += '<div class="row">';
      inlineQty +=
        '<div class="col-xs-4 zee_visit"><div class="input-group"><span class="input-group-addon" id="zee_visit_text">VISITED BY FRANCHISEE? </span><select id="zee_visit" class="form-control zee_visit" ><option></option>';
      ...
      inlineQty += '</select></div></div>';
      inlineQty +=
        '<div class="col-xs-6 zee_notes"><div class="input-group"><span class="input-group-addon" id="zee_notes_text">FRANCHISEE VISIT NOTES </span><input type="text" id="zee_notes" class="form-control zee_notes" value="' +
        zee_visit_notes + '" /></div></div>';
      inlineQty += '</div>';
      inlineQty += '</div>';
      ...
      if (zee != 696179 || (zee == 696179 && role != 1000)) {
        inlineQty +=
          '<div class="col-xs-4 survey1"><div class="input-group"><span class="input-group-addon" id="survey1_text">Using Mail / Parcels / Satchels Regularly? </span><select id="survey1" class="form-control survey1"><option></option>';

      } else if (zee == 696179 && role == 1000) {
        inlineQty +=
          '<div class="col-xs-4 survey1"><div class="input-group"><span class="input-group-addon" id="survey1_text">Using Mail / Parcels / Satchels Regularly? <span class="mandatory">*</span></span><select id="survey1" class="form-control survey1" required><option></option>';
      }
      ...
      if (zee != 696179 || (zee == 696179 && role != 1000)) {
        inlineQty +=
          '<div class="col-xs-6 survey7"><div class="input-group"><span class="input-group-addon" id="survey7_text">Frequency of Mail / Parcels / Satchels? </span><select id="survey7" class="form-control survey7"><option></option>';

      } else if (zee == 696179 && role == 1000) {
        inlineQty +=
          '<div class="col-xs-6 survey7"><div class="input-group"><span class="input-group-addon" id="survey7_text">Frequency of Mail / Parcels / Satchels? <span class="mandatory">*</span> </span><select id="survey7" class="form-control survey7" required><option></option>';
      }
      ...
      if (zee != 696179 || (zee == 696179 && role != 1000)) {
        inlineQty +=
          '<div class="col-xs-4 survey2"><div class="input-group"><span class="input-group-addon" id="survey2_text">Using Express Post? </span><select id="survey2" class="form-control survey2"><option></option>';

      } else if (zee == 696179 && role == 1000) {
        inlineQty +=
          '<div class="col-xs-4 survey2"><div class="input-group"><span class="input-group-addon" id="survey2_text">Using Express Post? <span class="mandatory">*</span></span><select id="survey2" class="form-control survey2" required><option></option>';
      }
      ...
      if (zee != 696179 || (zee == 696179 && role != 1000)) {
        inlineQty +=
          '<div class="col-xs-4 survey3"><div class="input-group"><span class="input-group-addon" id="survey3_text">Using Local Couriers? </span><select id="survey3" class="form-control survey3"><option></option>';

      } else if (zee == 696179 && role == 1000) {
        inlineQty +=
          '<div class="col-xs-4 survey3"><div class="input-group"><span class="input-group-addon" id="survey3_text">Using Local Couriers? <span class="mandatory">*</span></span><select id="survey3" class="form-control survey3"><option></option>';
      }
      ...
      inlineQty += '<div class="row">';
      inlineQty +=
        '<div class="col-xs-4 survey4"><div class="input-group"><span class="input-group-addon" id="survey4_text">Using P.O. Box? </span><select id="survey4" class="form-control survey4"><option></option>';
      ...
      inlineQty += '</select></div></div>';
      inlineQty +=
        '<div class="col-xs-4 survey5"><div class="input-group"><span class="input-group-addon" id="survey5_text">Bank Visit? </span><select id="survey5" class="form-control survey5"><option></option>';
      ...
      inlineQty += '</select></div></div>';
      inlineQty +=
        '<div class="col-xs-4 survey6"><div class="input-group"><span class="input-group-addon" id="survey6_text">Classify Lead </span><select id="survey6" class="form-control survey6"><option></option>';
    ...
    */
    function serviceDetailsSection(pricing_notes, ampo_price, ampo_time,
      ...
      if (role != 1000 && customer_list_page == null) {
        ...
        inlineQty += '<div class="row">';
        inlineQty +=
          '<div class="col-xs-6 auto_allocate_div"><div class="input-group"><span class="input-group-addon" id="auto_allocate_text">AUTO ALLOCATE</span><select id="auto_allocate" class="form-control auto_allocate"><option value="' +
          1 + '">YES</option><option value="' + 2 +
        ...
        inlineQty += '<div class="row">';
        inlineQty +=
          '<div class="col-xs-6 auto_allocate_div"><div class="input-group"><span class="input-group-addon" id="auto_allocate_text">AUTO ALLOCATE</span><select id="auto_allocate" class="form-control auto_allocate" disabled><option value="' +
          1 + '">YES</option><option value="' + 2 +
      ...
      inlineQty += '<div class="row">';
      inlineQty += '<div class="col-xs-6 heading3"><h4><span class="label label-default col-xs-12" style="background-color: #095c7b;">SERVICES PERFORMED</span></h4></div>';
      inlineQty += '<div class="col-xs-6 heading3"><h4><span class="label label-default col-xs-12" style="background-color: #095c7b;">FINANCIAL TAB</span></h4></div>';
      inlineQty += '</div>';
      inlineQty += '</div>';
      inlineQty += '<div class="form-group container service_section">';
      inlineQty += '<div class="row">';
      inlineQty += '<div class="col-xs-6 service_div">';
      inlineQty += '<table border="0" cellpadding="15" id="service" class="table table-responsive table-striped service tablesorter" cellspacing="0" style="width: 100%;font-size: 10px;"><thead style="color: white;background-color: #607799;"><tr><th style="vertical-align: middle;text-align: center;"><b>SERVICE NAME</b></th><th style="vertical-align: middle;text-align: center;"><b>SERVICE DESCRIPTION</b></th><th style="vertical-align: middle;text-align: center;"><b>SERVICE PRICE</b></th></tr></thead><tbody>';
      ...
      inlineQty += '<div class="col-xs-6 financial_tab_div">';
      inlineQty += '<table border="0" cellpadding="15" id="financial_tab" class="table table-responsive table-striped financial_tab tablesorter" cellspacing="0" style="width: 100%;font-size: 10px;"><thead style="color: white;background-color: #607799;"><tr><th style="vertical-align: middle;text-align: center;"><b>ITEM NAME</b></th><th style="vertical-align: middle;text-align: center;"><b>ITEM PRICE</b></th></tr></thead><tbody>';
      ...
      if (customer_record.getLineCount({ sublistId: 'itempricing' }) > 0) {
        for (var i = 0; i < customer_record.getLineCount({ sublistId: 'itempricing' }); i++) {
          ...
          inlineQty += '<td><div class="service_name_div"><input id="service_name" class="form-control service_name_current" readonly value="' + itemText + '" /></div></td>';
          inlineQty += '<td><div class="service_price_div input-group"><span class="input-group-addon">$</span><input class="form-control old_service_price_class" disabled value="' + itemPrice + '"  type="number" step=".01" /></div></td>';
          inlineQty += '</tr>';
      ...
      // inlineQty += '<div class="form-group container financial_tab_section">';
      // inlineQty += '<div class="row">';
      // inlineQty += '<div class="col-xs-12 heading3"><h4><span class="label label-default col-xs-12" style="background-color: #095c7b;">FINANCIAL TAB</span></h4></div>';
      // inlineQty += '</div>';
      // inlineQty += '</div>';
      // inlineQty += '<div class="form-group container sfinancial_tab_section">';
      // inlineQty += '<div class="row">';
      // inlineQty += '<div class="col-xs-12 financial_tab_div">';
      // inlineQty += '<table border="0" cellpadding="15" id="financial_tab" class="table table-responsive table-striped financial_tab tablesorter" cellspacing="0" style="width: 100%;"><thead style="color: white;background-color: #607799;"><tr><th style="vertical-align: middle;text-align: center;"><b>ITEM NAME</b></th><th style="vertical-align: middle;text-align: center;"><b>ITEM PRICE</b></th></tr></thead><tbody>';

      // log.debug({
      //   title: 'pricing count',
      //   details: customer_record.getLineCount({ sublistId: 'itempricing' })
      // })

      // log.debug({
      //   title: 'item name',
      //   details: customer_record.getSublistText({
      //     sublistId: 'itempricing',
      //     fieldId: 'item',
      //     line: 0
      //   })
      ...
      // log.debug({
      //   title: 'item price',
      //   details: customer_record.getSublistValue({
      //     sublistId: 'itempricing',
      //     fieldId: 'price',
      //     line: 0
      //   })
      ...
      inlineQty += '<div class="form-group container updateServiceFinancialTab_section">';
      inlineQty += '<div class="row">';
      inlineQty += '<div class="col-xs-3 "></div>';
      inlineQty +=
        '<div class="col-xs-6 updateServiceFinancialTab_div"><input type="button" value="UPDATE SERVICES & FINANCIAL TAB" class="form-control btn btn-primary" id="updateServiceFinancialTab" /></div>';
      inlineQty += '<div class="col-xs-3 "></div>';
      inlineQty += '</div>';
      inlineQty += '</div>';
      ...
      // }
      // inlineQty +=
      //   '</span><input id="ampo_price" class="form-control ampo_price" ';
      // if (role == 1000 && zee != 696179) {
      //   inlineQty += 'required';
      // }
      // if (!isNullorEmpty(ampo_price)) {
      //   inlineQty += ' value="' + ampo_price + '" data-oldvalue="' +
      //     ampo_price + '" /></div></div>';
      // } else {
      ...
      // }
      // inlineQty +=
      //   '</span><input id="pmpo_price" class="form-control pmpo_price"';
      // if (role == 1000 && zee != 696179) {
      //   inlineQty += ' required ';
      // }
      // if (!isNullorEmpty(pmpo_price)) {
      //   inlineQty += 'value="' + pmpo_price + '" data-oldvalue="' +
      //     pmpo_price + '" /></div></div>';
      // } else {
      //   inlineQty += 'value="" data-oldvalue="" /></div></div>';
      // }
    ...
    */
    function userNote(savedNoteSearch) {
      ...
      if (!isNullorEmpty(savedNoteSearch)) {
        ...
        savedNoteSearch.each(function (searchResult) {
          ...
          inlineQty += '<tr><td>' + note_date + '</td><td>' + author +
            '</td><td>' + message + '</td></tr>';
    ...
    function isFinanceRole(userRole) {
      ...
      return ((userRole == 1001 || userRole == 1031 || userRole == 1023) || (
        (userRole == 1032) || (userRole == 1006) || (userRole == 3)));
    }
